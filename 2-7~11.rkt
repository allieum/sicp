...
(define (mul-interval-wat x y)
  (define (handle-zero nonzero-endpoint other-interval)
    (let ((m (* nonzero-endpoint (lower-bound other-interval)))
          (n (* nonzero-endpoint (upper-bound other-interval))))
      (make-interval (min 0 m n) (max 0 m n))))
  ...
  (let ((a (lower-bound x))
    ...
    (cond ((or (both-pos? x y)
          ...
          ((has-zero-end? x)
           (handle-zero (get-nonzero a b) y))
          ((has-zero-end? y)
           (handle-zero (get-nonzero c d) x))
          ((and (neg? x) (< c 0))
